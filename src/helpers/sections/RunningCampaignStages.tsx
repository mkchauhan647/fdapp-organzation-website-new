...
const RunningCampaignStages : React.FC = () => {
  const { active_campaign_stages_data } = useAppSelector((state: RootState) => state.VotingCampaignStages);
  const { isFulfilled, isPending, fulfilledResponse } = active_campaign_stages_data;
  const RunningCampaignStageData: VotingCampaignStage[] = fulfilledResponse?.data.rows.slice(4,13);

  return (
    <CommonSection name='Running-campaign --has-active-campaigns'>
      <Heading title='Active Competition Stages' link='/campaigns' />
                      ...
                      <Skeleton isLoaded={isFulfilled && RunningCampaignStageData.length > 0 && !isPending} className="rounded-lg">
                        <div className="relative hover-card">
                          <Image
                            ...
                            src={process.env.NEXT_PUBLIC_AWS_URI + RunningCampaignStageData[(ind)*3].banner}
                            data-image={process.env.NEXT_PUBLIC_AWS_URI + RunningCampaignStageData[(ind)*3].banner}
                            width={400}
                            height={300}
                          />
                      ...
                      <Skeleton isLoaded={isFulfilled && RunningCampaignStageData.length > 0 && !isPending} className="rounded-lg">
                        <div className="relative hover-card">
                          <Image
                            alt="gallery"
                            className="block rounded-lg object-cover object-center height-auto w-[100%] "
                            src={process.env.NEXT_PUBLIC_AWS_URI + RunningCampaignStageData[(ind*3)+1]?.banner}
                            data-image={process.env.NEXT_PUBLIC_AWS_URI + RunningCampaignStageData[(ind*3)+1]?.banner}
                            width={400}
                            height={300}
                          />
                          <div className="card__overlay rounded-lg bg-black bg-opacity-75 text-white absolute bottom-0 left-0 right-0 p-[10px]">
                      ...
                      <Skeleton isLoaded={isFulfilled && RunningCampaignStageData.length > 0 && !isPending} className="rounded-lg">
                        {
                          ...
                          <div className="relative hover-card" data-id={RunningCampaignStageData[2+(ind*3)]?.id}>
                            <Image
                              alt="gallery"
                              className="block rounded-xl object-cover object-center aspect-[5/2]"
                              src={process.env.NEXT_PUBLIC_AWS_URI + RunningCampaignStageData[2+(ind*3)]?.banner}
                              width={800}
                              height={300}
                            />
...
export default RunningCampaignStages;
