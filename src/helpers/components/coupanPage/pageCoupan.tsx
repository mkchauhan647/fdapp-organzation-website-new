...
import { useAppDispatch, useAppSelector, RootState } from "@/helpers/hooks/useStoreHooks";
...
const CouponsPage: React.FC<CouponsPageProps> = ({ campaignID, candidateId }) => {
  ...
  const { all_candidates_by_campaign_id_data } = useAppSelector((state: RootState) => state.Candidates);
  ...
  const Candidates: Contestants[] = all_candidates_by_campaign_id_data?.fulfilledResponse?.data?.rows || [];
      ...
      <div className="w-full md:w-1/2 bg-white shadow-sm rounded-lg p-8 flex flex-col items-center">
        {candidate ? (
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4 max-w-[500px]">{candidate.biography}</p>
          </div>
        ) : (
