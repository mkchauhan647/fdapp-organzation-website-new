...
const CategorySlide: React.FC<{ id: string }> = ({ id }) => {
  ...
  const { all_campaign_stages_Data } = useAppSelector(
    (state: RootState) => state.VotingCampaignStages
  );
  const { isPending, isFulfilled, isRejected } = all_campaign_stages_Data;
  const fulfilledResponse = all_campaign_stages_Data.fulfilledResponse;

  let VotingCampaignStageData: VotingCampaignStage[] = id
    ? fulfilledResponse?.data.filter(
        (data: VotingCampaignStage) => data.votingCampaignId == id
      )
  ...
  return (
    <>
      ...
      <CommonSection name="category">
        ...
        {isPending ? (
          <div className="flex justify-between gap-3 items-baseline w-full overflow-hidden flex-wrap">
            {Array.from({ length: VotingCampaignStageData?.length || 3}).map((_, index) => (
              <div key={index} className="flex w-1/4">
                <SkeletonCampaign isloading={isPending} />
              </div>
            ))}
          </div>
        ) : (
            ...
            {VotingCampaignStageData?.length == 0 &&
              Array.from({ length: 4 }).map((_, index) => {
                return (
                  <SwiperSlide key={index}>
                    <SkeletonCampaign
                      isloading={VotingCampaignStageData.length != 0}
                    />
                  </SwiperSlide>
                );
              })}
