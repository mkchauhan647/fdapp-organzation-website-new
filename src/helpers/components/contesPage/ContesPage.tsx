      ...
      <div className="w-full pb-[70px]">
        <div className="container mx-auto flex flex-col justify-center">
          <div className="w-full flex flex-col md:flex-row justify-between items-start  mb-[30px] md:mb-[65px]">
            <p className="text-lg text-[var(--lightblack)] mb-4 sm:mb-0">
              Short by:{" "}
              <span className="text-lg text-[var(--blue)]">Newest</span>
            </p>
          ...
          {CandidatesData && CandidatesData.length > 0 && isFulfilled && (
            <Pagination
              itemsPerPage={12}
              items={CandidatesData.filter((contestant: Contestants) =>
                filterData.trim() !== ""
                  ? contestant.name?.includes(filterData)
                  : true
              )}
              ItemsComponent={({
                currentItems,
              }: {
                currentItems: Contestants[];
              }) => (
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-6 gap-x-4 sm:gap-x-16 md:gap-x-10 lg:gap-y-30 md:gap-y-8 gap-y-10 mb-[50px]">
                  {currentItems.map(
                    (contestant: Contestants, index: number) => (
                      <ContestantBox key={index} contestants={contestant} />
                    )
                  )}
                </div>
              )}
